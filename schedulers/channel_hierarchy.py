...
class ChannelHierarchyScheduler(DDPMScheduler):
    def __init__(
        ...
        codebook_channel_schedule: Optional[Dict[int, int]] = None,
        *args,
        **kwargs,
    ):
        ...
        if codebook_channel_schedule:
            self.codebook_channel_schedule = codebook_channel_schedule
        else:
            self.codebook_channel_schedule = {0: self.codebook_dims[0]}
    ...
    def step(
        ...
        codebook_channels = self._get_current_channel_config(step_index)
        codebook_t = self.codebook[step_index][:, :codebook_channels, :, :].to(device=model_output.device)
        ...
        if transformed_channels % codebook_channels != 0:
            raise ValueError(
                ...
                f"{transformed_channels} must be a multiple of codebook {codebook_channels}."
            )
